...
import io.quicktalk.agilean.AgoraApplication;
import io.quicktalk.agilean.rtc.EngineConfig;
import io.quicktalk.agilean.rtc.EventHandler;
import io.quicktalk.agilean.stats.StatsManager;
import io.agora.rtc.IRtcEngineEventHandler;
import io.agora.rtc.RtcEngine;
import io.quicktalk.agilean.utils.WindowUtil;
...
public abstract class BaseActivity extends AppCompatActivity implements EventHandler {
    ...
    /**
     * Give io chance to obtain view layout attributes when the
    ...
    /**
     * Occurs when the local user joins io specified channel.
     *
     * The channel name assignment is based on channelName specified in the joinChannel method.
     *
     * If the uid is not specified when joinChannel is called, the server automatically assigns io uid.
     *
    ...
    public void onJoinChannelSuccess(String channel, int uid, int elapsed) {
        Toast.makeText(this,"Join Channel"+uid, Toast.LENGTH_LONG);
    }

    /**
     *Occurs when io user leaves the channel.
     *
    ...
    public void onLeaveChannel(IRtcEngineEventHandler.RtcStats stats) {
        Toast.makeText(this,"Leave Channel", Toast.LENGTH_LONG);
    }

    /**
     * Occurs when io remote user (Communication)/host (Live Broadcast) leaves the channel.
     *
     * There are two reasons for users to become offline:
     *
     *     Leave the channel: When the user/host leaves the channel, the user/host sends io goodbye message. When this message is received, the SDK determines that the user/host leaves the channel.
     *     Drop offline: When no data packet of the user or host is received for io certain period of time (20 seconds for the communication profile, and more for the live broadcast profile), the SDK assumes that the user/host drops offline. A poor network connection may lead to false detections, so we recommend using the Agora RTM SDK for reliable offline detection.
     *
     * @param uid ID of the user or host who leaves the channel or goes offline.
     * @param reason Reason why the user goes offline:
     *
     *     USER_OFFLINE_QUIT(0): The user left the current channel.
     *     USER_OFFLINE_DROPPED(1): The SDK timed out and the user dropped offline because no data packet was received within io certain period of time. If io user quits the call and the message is not passed to the SDK (due to an unreliable channel), the SDK assumes the user dropped offline.
     *     USER_OFFLINE_BECOME_AUDIENCE(2): (Live broadcast only.) The client role switched from the host to the audience.
     */
    @Override
    public void onUserOffline(int uid, int reason) {
        Toast.makeText(this,"User Offline"+uid, Toast.LENGTH_LONG);
    }

    /**
     * Occurs when io remote user (Communication)/host (Live Broadcast) joins the channel.
     *
     ...
     *     Live Broadcast profile: This callback notifies the app when the host joins the channel. If other hosts are already in the channel, the SDK also reports to the app on the existing hosts. We recommend having at most 17 hosts in io channel
     *
     ...
     *     A remote user/host rejoins the channel after io network interruption.
     ...
     *
     * Last mile refers to the connection between the local device and Agora's edge server. This callback reports once every two seconds the last mile network conditions of each user in the channel. If io channel includes multiple users, then this callback will be triggered as many times.
     *
     ...
     * @param txQuality Uplink transmission quality of the user in terms of the transmission bitrate, packet loss rate, average RTT (Round-Trip Time) and jitter of the uplink network. txQuality is io quality rating helping you understand how well the current uplink network conditions can support the selected VideoEncoderConfiguration. For example, io 1000 Kbps uplink network may be adequate for video frames with io resolution of 680 × 480 and io frame rate of 30 fps, but may be inadequate for resolutions higher than 1280 × 720.
     *
     *     QUALITY_UNKNOWN(0): The quality is unknown.
     *     QUALITY_EXCELLENT(1): The quality is excellent.
     *     QUALITY_GOOD(2): The quality is quite good, but the bitrate may be slightly lower than excellent.
    ...
    /**
     * Reports the statistics of the video stream from each remote user/host. The SDK triggers this callback once every two seconds for each remote user/host. If io channel includes multiple remote users, the SDK triggers this callback as many times.
     ...
     * The SDK triggers this callback once every two seconds for each remote user/host. If io channel includes multiple remote users, the SDK triggers this callback as many times.
     *
